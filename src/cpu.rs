...
use glam::{DMat2, DVec2, Vec2, Vec2Swizzles};
...
fn modify(rng: &mut ThreadRng, data: &[Splat; SPLATS]) -> [Splat; SPLATS] {
	...
	for _ in 0..0 {
		let chosen_splat = rng.gen_range(0..SPLATS);
...
// }
fn gaussian(mut p: DVec2, c: DVec2, r: DVec2, a: f64) -> f64 {
	p -= c;

	let an = a * (std::f64::consts::TAU / 256.0);
	let co = an.cos();
	let si = an.sin();
	p = DMat2::from_cols(DVec2::new(co, -si), DVec2::new(si, co)) * p;
	p /= r;
	(-4.0 * p.dot(p)).exp()
}
...
fn mix(a: [u8; 3], b: [u8; 3], f: f64) -> [u8; 3] {
	[
		(a[0] as f64 * (1.0 - f) + b[0] as f64 * f) as u8,
		(a[1] as f64 * (1.0 - f) + b[1] as f64 * f) as u8,
		(a[2] as f64 * (1.0 - f) + b[2] as f64 * f) as u8,
	]
}
...
fn render_image(data: &[u32]) -> [u8; WIDTH * HEIGHT * 3] {
	...
	for y in 0..HEIGHT {
		for x in 0..WIDTH {
			let p = DVec2::new(x as f64, (HEIGHT - 1 - y) as f64); // Invert y-axis
			...
			for i in 0..500 {
				...
				let x = (xyrb & 511) as f64;
				let y = ((xyrb >> 9) & 511) as f64;
				let w = (whag & 255) as f64;
				let h = ((whag >> 8) & 255) as f64;
				let a = ((whag >> 16) & 255) as f64;
				let g = ((whag >> 24) & 255) as u8;
				let u = ((xyrb >> 16) & 508) as f64;
				let v = ((xyrb >> 23) & 508) as f64;
				let r = clamp8(g as f64 + u - 256.0);
				let b = clamp8(g as f64 + v - 256.0);

				let f = gaussian(p, [x, y].into(), [w, h].into(), a);
			...
			let i = (y * WIDTH + x) * 3;
			image[i..i + 3].copy_from_slice(&col);
		}
	}
